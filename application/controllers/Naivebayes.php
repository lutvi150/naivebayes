...
class Naivebayes extends CI_Controller
{
	...
	public function analisi_faktor_anak() {}
	...
	{
		$angka_pertama = $this->db->from('table_anak')->where('keterangan', 'SIAP')->count_all_results();

		$angka_kedua = 34;
		$response = [
			'anak_siap' => $angka_pertama,
			'P_siap' => round($angka_pertama / $angka_kedua, 2),
		];
		echo json_encode($response);
	}
	function prediksi()
	...
	function probilitas_akhir($id_anak)
	{
		$start = microtime(true);
		$id_sample = 3;
